...
만약 잘못된 인덱스를 입력하면 적절한 예외 메시지를 출력
...
6. 예외가 발생하지 않는 경우 "정상적으로 처리되었습니다."라는 메시지와 함께 해당 인덱스의 값을 출력
7. 프로그램 종료 메시지를 출력
8. 마이너스 인덱스 적용시키지 않음
'''
...
getattr(객체명, 속성명)함수 :
이상의 코드를 예시로 생각했을 때,
Person 클래스 내부에 있는 인스턴스 변수명을 그대로 집어넣는 것은 불가능하고 input을 통해서 받은 str 데이터를 대입했을 때 가능
