...
1. 파일 입출력의 개요
    ...
    파일 출력(output) : 기존 파일에 새로운 내용을 추가하거나 새로운 파일을 출력하는 것

    1) 파일 열기
        실제로 어떤 파일을 열어본다는 의미가 아니라
...
# file = open("hello.txt", "at")
# file.write("\nI didn't except this result.")
# file.close()

'''
기본 예제

오늘의 스케줄을 입력하면 그 내용을 모두 파일에 보관한는 프로그램을 작성
스케줄을 입력하지 않고 enter를 누르면 프로그램을 종료합니다.
생성되는 파일의 이름은 현재 날짜이고, 확장자는 txt입니다.
"2025-02-26"와 같은 형식을 갖추고 있습니다.
'''
# import time
#
...
'''
이상의 코드는 30 바이트씩 가지고 오게 되는데, 첫 번째 30 바이트의 결과값 이후에 자동으로 계행이 일어납니다. 그래서 
이부분을 파일과 콘솔에 정보가 동일하도록 end=""를 적용한 사례

    2)readline() 메서드
        텍스트 파일을 한 줄씩 읽어서 처리하는 메서드
        만ㅇ갸 파일이 종료되어 더 읽어들이 데이터가 없으면 빈 문자열을 읽음
        반복문을 이용해서 여러번 읽어들여야 할 때 파일 전체를 읽을 수 있음
...
'''
3) readlines()메서드
    전체 라인을 읽어들여서 각 라인 단위로 리스트에 저장하는 메서드
'''
...
nation = ["Greece", "Athene", "Germany", "Berlin", "Korea", "Seoul", "USA", "Washington D.C"]
