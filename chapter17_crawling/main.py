...
requests 라이브러리 : 파이썬에서 HTTP 요청을 보내기 위해 가장 널리 사용되는
    라이브러리 중 하나로, 사용하기 쉽고 직관적인 API를 재공하여
...
웹 크롤링이란 완성된 웹 페이지에서 필요한 정보를 수집하고 선별하여 추출하는 과정으로
...
<html>
    ...
    body
        ...
        인터넷 주소를 입력하는데 이때 입력한 주소가 URL에 해당함
'''
import requests
...
#
# print(f"응답 코드 : {requests.status_codes}")
# print(response.text)
...
2. 검색 결과 웹 페이지 정보를 가져오기
        ...
        검색하는 것이 함수 개념이라면 > 결과값은 return에 해당 검색어는 argument
        
        search("파이썬") > return값으로 검색 결과창
        
        입력한 검색어 "파이썬"은 query라는 매개변수로 전달
        search(query="파이썬")
...
'''
구글에서 파이썬을 검색한 결과를 가지고 오는 프로그램 완성
'''
